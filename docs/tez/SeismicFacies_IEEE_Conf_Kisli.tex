...
% --- Dil ve karakter kodlaması (XeLaTeX / Tectonic ile uyumlu) ------
% pdflatex için fontenc+inputenc gerek (eski derleyici), XeLaTeX
% için fontspec kullanılır. Aşağıdaki ifTeX yapısı her iki yolu destekler.
\usepackage{iftex}
\ifPDFTeX
  ...
  \shorthandoff{=}
\else
  % XeLaTeX / LuaLaTeX (Tectonic dahil) — native Unicode
  \usepackage{fontspec}
...
\usepackage{graphicx}
\usepackage{grffile}     % dosya adlarında alt çizgi/nokta için
\usepackage{booktabs}
\usepackage{array}
\usepackage{multirow}
...
FwIoU $0.832$) sırasıyla $+3.5$, $+5.6$ ve $+5.9$ puan üzerinde sonuç alınmıştır. Önemli bir
...
(Bölüm~\ref{sec:sfm}). Yapılan literatür taramasında SFM'in F3 fasiyes
veri kümesi üzerinde Alaudah coğrafi bölünme protokolüyle fine-tune
sonuçlarına rastlanmamıştır; bu çalışma söz konusu boşluğu hedeflemektedir.
Sistematik bir öncelik iddiası için daha kapsamlı bir literatür taraması
gerekmektedir.
...
\rowcolor{green!20}
\textbf{Bizim v9 Topluluk (3-seed)} \textbf{$\Rightarrow$ aktif final} & 2026 & Alaudah coğrafi & $\mathbf{0.791 \pm 0.006}$ & $\mathbf{0.940}$ & $\mathbf{0.873}$ & $\mathbf{0.877}$ & $\mathbf{0.891}$ \\
...
IoU}’da (FwIoU $0.891$ vs $0.832$, $+5.9$~puan) Alaudah 2019 en iyi
temel modelinin üzerinde sonuç verdiğini ortaya koymaktadır. FwIoU değeri,
test kümesi etiket dosyalarından doğrudan hesaplanan gerçek-değer (GT) sınıf
frekansları (Birleşik: S0: $\%21.5$, S1: $\%9.9$, S2: $\%50.7$, S3: $\%6.0$,
S4: $\%9.6$, S5: $\%2.3$) ile topluluk modelinin sınıf-bazlı IoU değerlerinin
ağırlıklı toplamı olarak hesaplanmıştır
(\texttt{scripts/compute\_fwiou\_ensemble.py}; Tablo~\ref{tab:fwiou_split}
Test1/Test2/Birleşik kırılımı). Üstelik bu karşılaştırma Alaudah'ın
\emph{orijinal coğrafi bölünmesine sadık kalan} ve veri sızıntılarından
arındırılmış (\emph{Yol~A} düzeltmesi) bir protokol üzerinde yapılmıştır.

\begin{table}[!t]
\renewcommand{\arraystretch}{1.2}
\caption{Topluluk Modelinin FwIoU Değerleri (Test Kümesi Kırılımı)}
\label{tab:fwiou_split}
\centering
\begin{tabular}{lc}
\toprule
\textbf{Test kümesi} & \textbf{FwIoU} \\
\midrule
Test1 (inline)    & $0.8888$ \\
Test2 (crossline) & $0.8964$ \\
\textbf{Birleşik} & $\mathbf{0.8908}$ \\
\midrule
Alaudah 2019 (Birleşik)~\cite{alaudah2019} & $0.832$ \\
\bottomrule
\end{tabular}
\end{table}
...
\subsection{Bileşen Ablation'ı}
\label{sec:ablation}

v9 mimarisinin üç tasarım kararının (Lovász-Softmax kaybı, 5-kanallı 2.5D
giriş, crossline-uyarlı augmentasyon) Birleşik ve Class~4 (Zechstein)
Test2 metriklerine etkisini ölçmek amacıyla bir bileşen ablation'ı
yürütülmüştür. Her ablation, v9 baseline (SEED=42) ile \emph{birebir aynı}
hiperparametrelerle eğitilmiş, yalnızca ilgili bileşen devre dışı
bırakılmıştır. Ablation maliyeti nedeniyle tek tohumla (SEED=42)
yürütülmüş, çoklu-tohum ablation gelecek iş olarak işaretlenmiştir.
Sonuçlar Tablo~\ref{tab:ablation}'de özetlenmiştir.

\begin{table}[!t]
\renewcommand{\arraystretch}{1.2}
\caption{Bileşen Ablation'ı --- v9 baseline (SEED=42) vs.\ tek-bileşen kapalı varyantlar}
\label{tab:ablation}
\centering
\begin{tabular}{lccc}
\toprule
\textbf{Konfigürasyon} & \textbf{Birleşik} & \textbf{$\Delta$ Birleşik} & \textbf{$\Delta$ C4 T2} \\
\midrule
v9 baseline (tam pipeline) & $0.7769$ & --- & --- (C4 T2 = $0.2304$) \\
$-$ Lovász (TripleLoss)    & $0.7792$ & $+0.23$ & $-1.43$ \\
$-$ 5-kanal $\rightarrow$ 3-kanal ($\pm 1$) & $0.7714$ & $-0.55$ & $\mathbf{-11.05}$ \\
$-$ xline-uyarlı aug       & $0.7688$ & $-0.81$ & $\mathbf{-9.88}$ \\
\bottomrule
\end{tabular}
\end{table}

Ablation üç bulguyu doğrulamaktadır: (i) \textbf{5-kanallı 2.5D giriş ve
crossline-uyarlı augmentasyon Class~4 Test2 performansı için olmazsa
olmaz} bileşenlerdir; her birinin kaldırılması Class~4 Test2 IoU'sunu
yaklaşık $10$~puan düşürmektedir. Bu, Class~4 odaklı tasarım kararlarının
\emph{post-hoc gerekçelendirmesi} değil \emph{niceliksel doğrulanması}dır.
(ii) \textbf{Lovász-Softmax kaybı} Birleşik mIoU üzerinde küçük negatif
etki gösterse de (\emph{TripleLoss}~$+0.23$~puan), doğrudan IoU surrogate
olarak Class~4 azınlık sınıfında $1.43$~puan tutarlı katkı sağlamaktadır.
(iii) Birleşik mIoU değişimleri ($\pm 0.81$~puan) sınıf-bazlı etkilerin
($\pm 11$~puan) çok altındadır; bu, agrege metriklerin azınlık-sınıf
gerilemelerini gizleyebildiğine işaret eder ve sınıf-bazlı raporlamanın
gerekliliğini ampirik olarak vurgulamaktadır.
...
\midrule
v9 Topluluk (ana)        & $\mathbf{0.791}$ & $\mathbf{0.940}$ & $\mathbf{0.873}$ & $\mathbf{0.891}$ & $0.183$ \\
...
\textit{Önemli çekince.} SFM v1 ve v2 sonuçları tek tohum (SEED=42) ile
elde edilmiştir; SFM ailesi için multi-seed ablation yapılmamıştır.
Bölüm~\ref{sec:c4_aykiri}'de v9 modeli için somutlanan tek-tohum aykırı
değer riski (Class~4 Test2 IoU $0.230$'un üç-tohum gerçek değeri
$0.183 \pm 0.04$ olarak görüldü) prensip olarak SFM ölçümleri için de
geçerlidir. Bu nedenle SFM~v1'in Class~4 Test2'deki $0.270$ üstünlüğü,
\emph{yüksek varyans bandında değerlendirilmesi gereken} potansiyel bir
bulgu olarak raporlanmaktadır; istatistiksel olarak doğrulanmış bir
üstünlük iddiası için en az üç-tohumlu SFM tekrarı gerekmektedir ve
bu, çoklu-tohum ablation paketiyle birlikte gelecek iş olarak
işaretlenmiştir.
...
\subsection{Bileşen Katkılarının Yorumu}

Bölüm~\ref{sec:ablation}'de raporlanan ablation sonuçları, v9 tasarımının
üç bileşen kararının da \emph{Class~4 odaklı} olduğunu niceliksel olarak
ortaya koymaktadır: 5-kanallı 2.5D giriş (Class~4 Test2 üzerinde
$-11.05$~puan) ve crossline-uyarlı augmentasyon ($-9.88$~puan) Class~4
Zechstein crossline morfolojisi için \emph{kritik} bileşenler iken,
Lovász-Softmax katkısı küçük ama tutarlı kalmaktadır ($-1.43$~puan).
Birleşik mIoU'daki değişimlerin sınıf-bazlı etkilerin çok altında kalması
($\pm 0.81$ vs.\ $\pm 11$ puan), agrege metriklerin azınlık-sınıf
gerilemelerini gizleyebildiğini ampirik olarak göstermektedir; bu, sismik
fasiyes literatüründe yalnızca birleşik metrik raporlamanın
yetersizliğine işaret eden bağımsız bir bulgudur.
...
denenmedi.
\item \textbf{Ablasyon tek-tohum.} Bölüm~\ref{sec:ablation}'de raporlanan
bileşen ablation'ı (Lovász, 5-kanal 2.5D, crossline-uyarlı augmentasyon)
tek tohumla (SEED=42) yürütülmüştür; bulgular ana eğilim göstergesi
niteliğindedir ve ablation kararlarına ilişkin güven aralığı
hesaplanmamıştır. TTA varyant ablation'ı (multi-scale vs.\ HFlip-only
vs.\ no-TTA) ile Mixup, Focal $\alpha$ ve etiket-yumuşatma parametrelerinin
tek-tek ablation'ı bu çalışma kapsamı dışında bırakılmış, çoklu-tohum
ablation paketiyle birlikte gelecek iş olarak işaretlenmiştir.
...
modelinin PA ($+3.5$~puan), MCA ($+5.6$~puan) ve FwIoU ($+5.9$~puan)
...
elde edilmiştir. FwIoU değerleri Alaudah formülü
($\text{FwIoU} = \sum_{c} f_c \cdot \text{IoU}_c$) ile hesaplanmıştır;
sınıf frekansları $f_c$ test kümesi etiket dosyalarındaki (\texttt{test1\_labels.npy}
$\cup$ \texttt{test2\_labels.npy}) gerçek-değer piksel sayımlarından, sınıf-bazlı
IoU değerleri topluluk modelinin Birleşik karmaşıklık matrisinden alınmıştır
(\texttt{scripts/compute\_fwiou\_ensemble.py}). Hesaplama betiği JSON
çıktılarından doğrudan yeniden üretilebilir. Veri seti Zenodo
